...
export class KerasModel extends Widget {
  ...
  constructor(widget_id?: string) {
    ...
    if (widget_id) {
      super({ node: KerasModel.createNode(widget_id) });
    } else {
      super({ node: KerasModel.createNode(uuid()) });
    }
  }
  static createNode(widget_id: string): HTMLElement {
    ...
    node.innerHTML =
      '<div class="head">' +
      '<div class="left">Tensorflow/Keras/Model' +
      '</div>' +
      '<div class="right">' +
      ...
      '</div>' +
      '<div class="body">' +
      '<input id = "model_info" type="text" name="model_info" placeholder="Model details description">' +
      '</div>';

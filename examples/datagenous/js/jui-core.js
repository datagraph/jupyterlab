var jui = (function($) {
  ...
  var utility = (global$1['util.base'] = {
    ...
    browser: {
      webkit:
        'WebkitAppearance' in document.documentElement.style ? true : false,
     ...
     * check touch device
     */
    isTouch: /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(
    ...
    inherit: function inherit(ctor, superCtor) {
      if (
        !this.typeCheck('function', ctor) ||
        !this.typeCheck('function', superCtor)
      )
        return;
    ...
    typeCheck: function typeCheck(t, v) {
      ...
      if (
        (typeof t === 'undefined' ? 'undefined' : _typeof(t)) == 'object' &&
        t.length
      ) {
        var typeList = t;
    ...
    typeCheckObj: function typeCheckObj(uiObj, list) {
      if (
        (typeof uiObj === 'undefined' ? 'undefined' : _typeof(uiObj)) !=
        'object'
      )
        return;
      var self = this;
            ...
            uiObj[funcName] = function() {
              ...
              for (var i = 0; i < args.length; i++) {
                if (!self.typeCheck(params[i], args[i])) {
                  throw new Error(
                    'JUI_CRITICAL_ERR: the ' +
                      i +
                      'th parameter is not a ' +
                      params[i] +
                      ' (' +
                      name +
                      ')'
      ...
      var opts = this.extend(
        {
          ...
          names: null,
          types: null,
          count: this.typeCheck('integer', options.count)
            ? options.count
            : options.rows.length
        },
        options
      ...
      for (var i = 1; i < tmpRowArr.length; i++) {
        if (tmpRowArr[i] != '') {
          ...
          for (var j = 0; j < keys.length; j++) {
            ...
            // '"' 로 감싸져있는 문자열은 '"' 제거
            if (
              this.startsWith(tmpArr[j], '"') &&
              this.endsWith(tmpArr[j], '"')
            ) {
    ...
    inArray: function inArray(target, list) {
      if (
        this.typeCheck(['undefined', 'null'], target) ||
        !this.typeCheck('array', list)
      )
        return -1;
    ...
    param: function param(data) {
      var r20 = /%20/g,
        ...
        add = function add(key, value) {
          ...
          value = utility.typeCheck('function', value)
            ? value()
            : value == null
            ? ''
            : value;
    ...
    ajax: function ajax(data) {
      ...
      if (
        !this.typeCheck('string', opts.url) ||
        !this.typeCheck('function', opts.success)
      )
        return;
  ...
  var jui = {
    ...
    ready: function ready() {
      ...
      if (
        !utility.typeCheck(['array', 'null'], depends) ||
        !utility.typeCheck('function', callback)
      ) {
        throw new Error(
          'JUI_CRITICAL_ERR: Invalid parameter type of the function'
        );
      }
    ...
    defineUI: function defineUI(name, depends, callback, parent) {
      if (
        !utility.typeCheck('string', name) ||
        !utility.typeCheck('array', depends) ||
        !utility.typeCheck('function', callback) ||
        !utility.typeCheck(['string', 'undefined'], parent)
      ) {
        throw new Error(
          'JUI_CRITICAL_ERR: Invalid parameter type of the function'
        );
    ...
    createUIObject: function createUIObject(
      UI,
      selector,
      index,
      elem,
    ...
    define: function define(name, depends, callback, parent) {
      if (
        ...
        !utility.typeCheck('array', depends) ||
        !utility.typeCheck('function', callback) ||
        !utility.typeCheck(['string', 'undefined', 'null'], parent)
      ) {
        throw new Error(
          'JUI_CRITICAL_ERR: Invalid parameter type of the function'
        );
    ...
    defineOptions: function defineOptions(Module, options, exceptOpts) {
      ...
      for (var i = 0; i < optKeys.length; i++) {
        ...
        if (
          utility.inArray(name, defOptKeys) == -1 &&
          utility.inArray(name, exceptOpts) == -1
        ) {
          ...
          if (module.extend != null) {
            if (jui.include(module.extend) == null) {
              console.warn(
                "JUI_WARNING_MSG: '" +
                  module.extend +
                  "' module should be imported in first"
              );
            }
          }
      ...
      return {
        find: function find() {
          ...
          if (args.length == 1) {
            ...
            if (
              _.typeCheck('object', args[0]) &&
              _.typeCheck('string', args[1])
            ) {
      ...
      var QuickSort = function QuickSort(array, isClone) {
        ...
        function partition(pivot, left, right) {
          ...
          for (var v = left; v < right; v++) {
            if (
              compareFunc(array[v], pivotValue) ||
              (!compareFunc(pivotValue, array[v]) && v % 2 == 1)
            ) {
              swap(v, storeIndex);
              storeIndex++;
            }
      ...
      var self = {
        ...
        nice: function nice(min, max, ticks, isNice) {
          ...
          function caculate() {
            ...
            _tickSpacing = isNice
              ? niceNum(_range / _ticks, true)
              : _range / _ticks;
            _niceMin = isNice
              ? Math.floor(_min / _tickSpacing) * _tickSpacing
              : _min;
            _niceMax = isNice
              ? Math.floor(_max / _tickSpacing) * _tickSpacing
              : _max;
          }
      ...
      var self = {
        ...
        map: function map(color_list, count) {
          ...
          for (var i = 0, len = color_list.length - 1; i < len; i++) {
            if (i == 0) {
              colors = scale
                .domain(color_list[i], color_list[i + 1])
                .ticks(count);
            } else {
              var colors2 = scale
                .domain(color_list[i], color_list[i + 1])
        ...
        lighten: function lighten(color, rate) {
          ...
          for (i = 0; i < 6; i += 2) {
            ...
            c = Math.round(Math.min(Math.max(0, c + c * rate), 255)).toString(
              16
            );
        ...
        parseStop: function parseStop(stop) {
          ...
          for (var i = 0, len = stop_list.length; i < len; i++) {
            ...
            if (arr.length == 1) {
              ...
              stops.push({
                type: 'stop',
                attr: { offset: arr[0], 'stop-color': arr[1] }
              });
          ...
          for (var i = 0, len = stops.length; i < len; i++) {
            ...
            if (start == -1 && typeof stop.offset == 'undefined') {
              ...
              var endOffset =
                stops[end].offset.indexOf('%') > -1
                  ? parseFloat(stops[end].offset) / 100
                  : stops[end].offset;
              var startOffset =
                stops[start].offset.indexOf('%') > -1
                  ? parseFloat(stops[start].offset) / 100
                  : stops[start].offset;
    ...
    component: function component() {
      ...
      var UICore = function UICore() {
        ...
        this.on = function(type, callback) {
          if (
            !_.typeCheck('string', type) ||
            !_.typeCheck('function', callback)
          )
            return;
          this.event.push({
        ...
        this.off = function(type) {
          ...
          for (var i = 0; i < this.event.length; i++) {
            ...
            if (
              (_.typeCheck('function', type) && e.callback != type) ||
      ...
      UICore.init = function(UI) {
        ...
        if (
          (typeof UI === 'undefined' ? 'undefined' : _typeof(UI)) === 'object'
        ) {
      ...
      UIEvent.build = function(UI) {
        return function(selector, options) {
          ...
          $root.each(function(index) {
            list[index] = jui.createUIObject(
              UI,
              $root.selector,
              index,
              this,
              options,
              function(mainObj, opts) {
                /** @property {Object} listen Dom events */
                mainObj.init.prototype.listen = new DOMEventListener();

                $('script').each(function(i) {
                  if (
                    selector == $(this).data('jui') ||
                    selector == $(this).data('vo') ||
                    ...
                    if (tplName == '') {
                      throw new Error(
                        "JUI_CRITICAL_ERR: 'data-tpl' property is required"
                      );
                    }
